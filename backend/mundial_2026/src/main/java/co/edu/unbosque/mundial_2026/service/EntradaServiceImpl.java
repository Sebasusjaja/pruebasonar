...
public class EntradaServiceImpl implements EntradaService {
    ...
    public EntradaResponseDTO reservarEntrada(String correo, EntradaRequestDTO dto) {
        ...
        validarCapacidadDisponible(partido, dto.getCantidad());
        validarLimitePorTransaccion(dto.getCantidad());
        validarLimiteDiario(usuarioId, dto.getCantidad());
        Entrada entrada = crearEntradaReservada(u, partido, dto.getCantidad());
    ...
    public EntradaResponseDTO confirmarPago(Long entradaId, String paymentRef) {
        ...
        validarEstadoReservada(entrada);
        validarTtlReserva(entrada);
        procesarPagoStripe(entrada, paymentRef, entradaId);
        return toDTO(entrada);
    }
    ...
    public EntradaResponseDTO cancelarReserva(String correo, Long entradaId) {
        ...
        Entrada entrada = entradaRepository.findById(entradaId)
                .orElseThrow(() -> new EntradaNotFoundException(ENTRADA_NO_ENCONTRADA));
        validarPropietario(entrada, usuarioId);
        validarEstadoEsReservada(entrada);
    ...
    public EntradaResponseDTO transferirEntrada(Long entradaId, TransferenciaRequestDTO dto, String correo) {
        ...
        Entrada entrada = entradaRepository.findById(entradaId)
                .orElseThrow(() -> new EntradaNotFoundException(ENTRADA_NO_ENCONTRADA));
        validarPropietario(entrada, usuarioId);
        validarEstadoEsPagada(entrada);
        validarLimiteDiarioTransferencia(usuarioId, entrada.getCantidad());
        Usuario usuarioRecibe = usuarioService.obtenerEntidadPorCorreo(dto.getCorreoDestino());
        entrada.setEstado(ESTADO_TRANSFERIDA);
        entradaRepository.save(entrada);
        Entrada nuevaEntrada = crearEntradaTransferida(entrada, usuarioRecibe);
    ...
    public EntradaResponseDTO reembolsarEntrada(String correo, Long entradaId) {
        ...
        validarPropietario(entrada, usuarioId);
        validarEstadoEsPagada(entrada);
        procesarReembolsoStripe(entrada, entradaId, usuarioId);
        return toDTO(entrada);
    }
    ...
    public EntradaResponseDTO obtenerEntrada(Long entradaId) {
        Entrada entrada = entradaRepository.findById(entradaId)
                .orElseThrow(() -> new RuntimeException(ENTRADA_NO_ENCONTRADA));
        return toDTO(entrada);
    }
    ...
    private void validarCapacidadDisponible(Partido partido, int cantidad) {
        if (partido.getCapacidadDisponible() < cantidad) {
            throw new CupoNoDisponibleException("No hay cupo disponible para este partido");
        }
    }

    private void validarLimitePorTransaccion(int cantidad) {
        if (cantidad > 4) {
            throw new LimiteSuperadoException("Máximo 4 entradas por transacción");
        }
    }

    private void validarLimiteDiario(Long usuarioId, int cantidad) {
        ...
        if (compradasHoy + cantidad > 12) {
            throw new LimiteSuperadoException("Límite diario de 12 entradas alcanzado");
        }
    }

    private void validarLimiteDiarioTransferencia(Long usuarioId, int cantidad) {
        LocalDateTime inicioDia = LocalDateTime.now().toLocalDate().atStartOfDay();
        LocalDateTime finDia = inicioDia.plusDays(1);
    ...
    private void procesarReembolsoStripe(Entrada entrada, Long entradaId, Long usuarioId) {
        try {
